...
export default function Layout() {
  ...
  const onSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchQuery.trim()) { navigate(`/discover?search=${encodeURIComponent(searchQuery.trim())}`); setSearchQuery('') }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <form onSubmit={onSearch} className={`hidden md:flex items-center flex-1 max-w-md mx-6 rounded-full px-4 py-2 transition-all ${
              ...
              <input type="text" placeholder="Search venues, cuisines..." value={searchQuery}
                ...
                {cityOpen && (
                  <>
                    ...
                    <div className="absolute top-full right-0 mt-2 w-56 glass-strong rounded-2xl py-2 z-50">
                      ...
                      {cities?.map(c => (
                        <button key={c.id} onClick={() => { navigate(`/discover?city=${c.id}`); setCityOpen(false) }}
              ...
              {isAuthenticated ? (
                <div className="flex items-center gap-1 ml-2">
                  <Link to="/profile" className={`flex items-center gap-2 px-3 py-2 rounded-full text-sm font-medium transition-colors ${navText} ${navTextHover}`}>
                    {user?.avatar ? <img src={user.avatar} alt="" className="w-7 h-7 rounded-full object-cover" /> : <User className="w-5 h-5" />}
            ...
            <button onClick={() => setMobileOpen(!mobileOpen)} className={`md:hidden p-2 rounded-lg transition-colors ${scrolled || !isHome ? 'text-gray-700 hover:bg-gray-100' : 'text-white hover:bg-white/10'}`}>
          ...
          <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-gray-100 shadow-lg">
            <div className="px-4 py-4 space-y-1">
              <form onSubmit={onSearch} className="flex items-center bg-gray-100 rounded-full px-4 py-2.5 mb-3">
                ...
                <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-transparent outline-none text-sm w-full text-gray-900" />
              </form>
              {[{to:'/discover',label:'Discover',icon:Compass},{to:'/plans',label:'Day Plans',icon:CalendarDays},{to:'/surprise-me',label:'Surprise Me',icon:Zap},{to:'/events',label:'Events',icon:PartyPopper},{to:'/deals',label:'Deals',icon:Ticket},{to:'/outreach',label:'Outreach',icon:Send},{to:'/my-plan',label:'My Plan',icon:ShoppingBag}].map(item => (
              ...
              {cities?.slice(0,5).map(c => (
                <button key={c.id} onClick={() => navigate(`/discover?city=${c.id}`)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-700 hover:bg-gray-50 text-sm text-left">
              ...
              <div className="border-t border-gray-100 pt-3 mt-2">
                {isAuthenticated ? (
                  <>
                    <Link to="/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-700 hover:bg-gray-50 text-sm"><User className="w-5 h-5 text-[#FF6B4A]" />My Profile</Link>
                    <button onClick={logout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-700 hover:bg-gray-50 text-sm"><LogOut className="w-5 h-5 text-[#FF6B4A]" />Sign Out</button>
                  </>
                ) : (
                  <Link to="/login" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[#FF6B4A] font-medium text-sm"><LogIn className="w-5 h-5" />Sign In</Link>
                )}
              </div>
            </div>
          </div>
      ...
      <footer className="bg-[#1A1A2E] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg bg-coral-gradient flex items-center justify-center"><Compass className="w-5 h-5 text-white" /></div>
            ...
            <div>
              ...
              <ul className="space-y-2">{cities?.slice(0,6).map(c => (<li key={c.id}><button onClick={() => navigate(`/discover?city=${c.id}`)} className="text-sm text-gray-400 hover:text-white transition-colors">{c.name}</button></li>))}</ul>
            </div>
            <div>
          ...
          <div className="border-t border-white/10 mt-10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-1 text-xs text-gray-500"><Heart className="w-3 h-3 text-[#FF6B4A]" /> Made with care in Canada</div>
          </div>
        </div>
      </footer>
    </div>
